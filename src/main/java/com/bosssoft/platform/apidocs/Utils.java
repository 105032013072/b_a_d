        ...
        Utils.wideSearchFile(javaSrcDir, new FilenameFilter() {
            ...
            public boolean accept(File f, String name) {
                if(name.endsWith(".java")){
                	NodeList<ImportDeclaration> list= ParseUtils.compilationUnit(f).getImports();
                	for (ImportDeclaration im : list) {
						if(im.getNameAsString().contains("org.springframework.web")) return true;
					}
                }
                return false;
		...
		Utils.wideSearchFile(javaSrcDir, new FilenameFilter() {
			...
			public boolean accept(File f, String name) {
				if(name.endsWith(".java")){
					NodeList<ImportDeclaration> list=ParseUtils.compilationUnit(f).getImports();
					for (ImportDeclaration im : list) {
						if(im.getNameAsString().contains("com.jfinal.core")) return true;
					}
				}
				return false;
